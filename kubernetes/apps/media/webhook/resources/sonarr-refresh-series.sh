...
function refresh() {
    if [[ "${SONARR_EVENT_TYPE}" == "Test" ]]; then
        ...
        episodes=$(
            curl -fsSL --header "X-Api-Key: ${SONARR_API_KEY}" "http://${SONARR_REMOTE_ADDR}/api/v3/episode?seriesId=${SONARR_SERIES_ID}" |
        ...
        if ((episodes > 0)); then
            ...
            curl -fsSL --request POST \
                ...
                --data-binary "$(jo name=RefreshSeries seriesId="${SONARR_SERIES_ID}")" \
