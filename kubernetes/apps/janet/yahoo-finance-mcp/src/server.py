# Vendored, UNMODIFIED from upstream Alex2Yang97/yahoo-finance-mcp
# (https://github.com/Alex2Yang97/yahoo-finance-mcp), commit
# 241e52316729474cabfb3e338a74c0e6ad4b25e0, MIT-licensed. yfinance-backed MCP
# server exposing 9 tools (historical prices, stock info, news, actions,
# financial statements, holders, options, recommendations).
#
# Not patched for HTTP here on purpose — see http_entrypoint.py, which imports
# the `yfinance_server` FastMCP instance below and serves it over
# streamable-HTTP without touching this file, so upstream diffs stay a clean
# drop-in replacement (re-vendor by re-copying server.py verbatim).
#
# `main()`/`if __name__ == "__main__"` below (stdio transport) are dead code
# in this deployment — the container's entrypoint is http_entrypoint.py, not
# this file — kept only so this stays a faithful vendor copy.
import json
